...
## Corpus Check
- 109 files · ~74,686 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 353 nodes · 388 edges · 16 communities detected
- Extraction: 75% EXTRACTED · 25% INFERRED · 0% AMBIGUOUS · INFERRED: 96 edges (avg confidence: 0.8)
...
5. `generateMetadata()` - 9 edges
6. `POST()` - 9 edges
7. `createAdminClient()` - 8 edges
8. `Souqly Project CLAUDE.md Context` - 8 edges
9. `getSiteUrl()` - 7 edges
10. `Souqly Brand Guide` - 7 edges

## Surprising Connections (you probably didn't know these)
- `generateMetadata()` --calls--> `createClient()`  [INFERRED]
  src\app\[slug]\(catalogue)\produit\[id]\page.tsx → src\lib\supabase\server.ts
- `GET()` --calls--> `generateMetadata()`  [INFERRED]
  src\app\auth\callback\route.ts → src\app\[slug]\(catalogue)\produit\[id]\page.tsx
- `generateMetadata()` --calls--> `formatPrice()`  [INFERRED]
  src\app\[slug]\(catalogue)\produit\[id]\page.tsx → src\lib\utils\format.ts
...
Cohesion: 0.06
Nodes (44): Client Final (Visitor) Actor, Super-Admin Actor, DB Table: access_sessions, DB Table: cart_submissions, DB Table: catalog_visits, DB Table: categories, DB Table: merchant_applications, DB Table: merchants (+36 more)
...
Cohesion: 0.13
Nodes (10): getAllPosts(), getPostBySlug(), getRelatedPosts(), BlogArticlePage(), formatDate(), generateMetadata(), generateStaticParams(), getCatalogData() (+2 more)
...
Cohesion: 0.32
Nodes (5): submitCartStats(), formatPrice(), generateOrderMessage(), buildOrderMessage(), handleOrder()
...
- **Thin community `Community 15`** (5 nodes): `proxy()`, `Testimonials.tsx`, `proxy.ts`, `next()`, `prev()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 88`** (1 nodes): `Public Pages Architecture`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 89`** (1 nodes): `Merchant Dashboard Pages`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 90`** (1 nodes): `Super-Admin Pages`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 91`** (1 nodes): `Explicitly Out of Scope Features`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `createClient()` connect `Community 1` to `Community 9`, `Community 2`, `Community 3`, `Community 7`?**
  _High betweenness centrality (0.039) - this node is a cross-community bridge._
- **Why does `generateMetadata()` connect `Community 3` to `Community 1`, `Community 9`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
- **Why does `Souqly Platform` connect `Community 0` to `Community 5`?**
  _High betweenness centrality (0.026) - this node is a cross-community bridge._
...
- **Are the 4 inferred relationships involving `generateMetadata()` (e.g. with `getPostBySlug()` and `createClient()`) actually correct?**
